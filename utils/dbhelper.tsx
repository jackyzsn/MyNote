    ...
    (await db).transaction((trans: Transaction) => {
        trans.executeSql(
            ...
            (tx: Transaction, results: ResultSet) => {
                ...
                let nowString = now.format('YYYY-MM-DDTHH:mm:ss.SSS');
                UUIDGenerator.getRandomUUID().then(uid => {
                    fetch(`${setting.mongodb.backupURL}/api/notes/`, {
                        method: 'POST',
                        headers: {
                            Accept: 'application/json',
                            'Content-Type': 'application/json'
...
export function retrieveBackupsMongo(
    ...
    fetch(`${setting.mongodb.backupURL}/api/notes/`, {
        ...
        .then(json => {
            let backupList = json.map((item: any) => ({
                uuid: item.uuid,
                device: item.device,
                backupAt: item.updt,
            }));
...
export function restoreToDBMongo(
    ...
    console.log(uuid);
    fetch(`${setting.mongodb.backupURL}/api/notes/${uuid}`, {
        method: 'GET',
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json'
        ...
        .then(async json => {
            let notes = JSON.parse(json.content);
